...
export default function ScriptlessBookingPanel() {
  ...
  const bookNow = () => {
    ...
    const payload = {
      ...
      from,
      to,
      date: dateStr,  // raw YYYY-MM-DD for machine use
      time: timeStr,  // raw HH:MM for machine use
    };

    // --- Preferred: GET with query params (works across 3000 → 3001) ---
    const url = buildBookingURL(BOOK_DEST, payload);
    window.location.href = url;

    // --- Optional: POST (hides details from URL). Only works same-origin or if CORS/form-POST is accepted.
    // const form = document.createElement('form');
    // form.method = 'POST';
    // form.action = BOOK_DEST;
    ...
    >
      ...
      ">
        ...
        <div className="mt-6 flex justify-center">
          <button
            onClick={bookNow}
            className="min-w-[200px] h-12 px-8 bg-[#F2B400] hover:bg-[#e0a600]
                       text-white font-semibold rounded-xl shadow-md transition text-base"
          >
            Book Now
          </button>
        </div>
      ...
      ">
        ...
        <button
          onClick={bookNow}
          className="w-full mt-5 h-12 px-8 bg-[#F2B400] hover:bg-[#e0a600]
                     text-white font-semibold rounded-xl shadow-md transition text-base"
        >
          Book Now
        </button>
...
function NativePicker({
  ...
  const openPicker = () => {
    ...
    // @ts-ignore - showPicker is supported in most modern browsers
